...
<div class="flex justify-center mb-8 max-w-screen-xl w-screen">
  ...
  >
    ...
    />
    <Button class="!p-2.5 bg-green-600" on:click={fetchData}>
    ...
    <div class="grid grid-cols-1 gap-4 md:gap-6 md:grid-cols-2 lg:grid-cols-3">
      {#each results as result (result._id)}
        ...
        >
          ...
          <!-- Details -->
          <p class="text-green-600"><strong>Nama:</strong> {result.Nama}</p>
          <p>
            <strong>Nama Pembimbing Kemasyarakatan:</strong>
            {result.NamaPK === "0" ? "-" : result.NamaPK}
          </p>
          <p>
            <strong>Nomor Register:</strong>
            {result.NoRegisterLitmas === "0" ? "-" : result.NoRegisterLitmas}
          </p>
          <p>
            <strong>Masa Bimbingan Akhir:</strong>
            {result.Katagori === "0" ? "-" : result.Katagori}
          </p>
          <p>
